...
def execute():
    ...
    # 出力先ワークブック指定
    wbc = WorkbookControl(target='./tests/temp/test_o5o0_multi_root_by_table.xlsx', mode='w')

    # ワークシート描画
    wbc.render_worksheet(target='Shiritori', based_on='./examples/data/multi_root_by_table.csv')
    WorksheetDumpControl.dump(worksheet=wbc._ws, file='./tests/diff_dump/actual/test_o5o0_multi_root_by_table_Shiritori.txt')     # テスト用
