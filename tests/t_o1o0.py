...
def execute():
    ...
    with tr.prepare_workbook(target='./tests/temp/test_o1o0_drive_by_tree.xlsx', mode='w') as b:
        ...
        with b.prepare_worksheet(target='DriveByTable', based_on='./examples/data/drive_by_table.csv') as s:
            ...
            # ワークシートへ木構造図を描画
            s.render_tree()
            WorksheetDumpHandle.dump(worksheet=s._ws, file='./tests/diff_dump/actual/test_o1o0_drive_by_table_DriveByTable.txt')     # テスト用
        ...
        with b.prepare_worksheet(target='DriveByTree', based_on='./examples/data/drive_by_tree.csv') as s:
            ...
            # ワークシートへ木構造図を描画
            s.render_tree()
            WorksheetDumpHandle.dump(worksheet=s._ws, file='./tests/diff_dump/actual/test_o1o0_drive_by_tree_DriveByTree.txt')     # テスト用
