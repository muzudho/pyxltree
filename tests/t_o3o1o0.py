...
def execute():
    ...
    with tr.prepare_workbook(target='./tests/temp/test_o3o1o0_word_chain_game_Drive.xlsx', mode='w', settings={
        # その他の操作
        ...
        with b.prepare_worksheet(target='WordChainGame', based_on='./examples/data/word_chain_game.csv') as s:
            ...
            # ワークシートへ木構造図を描画
            s.render_tree()
            WorksheetDumpHandle.dump(worksheet=s._ws, file='./tests/diff_dump/actual/test_o3o1o0_word_chain_game_Drive.txt')     # テスト用
